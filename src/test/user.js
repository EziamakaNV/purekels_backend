...
describe('POST /api/v1/auth/signup', function () {
  describe('When all values in the POST body are the right format', function () {
    ...
    it('Should return an error if the length of any of the paremeters is less than 3', function (done) { // Mocha done callback for asynchronous tests
      chai.request(server)
        .post('/api/v1/auth/signup')
        .type('form')
        .send({
  ...
  after(function (done) {
    usersCollection.findOneAndDelete({ email: 'tp12@work.com' })
      .then(function () { done(); })
      .catch(function (err) {
